...
handle_cast({message, Direction, From, To, Type, Body}, #state{iodevice = IoDevice} =
	    State) ->
    write_log(IoDevice, Direction, From, To, Type, Body),
    {noreply, State};
...
log_packet(Direction, From, To, #xmlel{name = <<"message">>} = Packet) ->
    Tag = xml:get_subtag(Packet, <<"body">>),
    case Tag of
      #xmlel{children = Body} when length(Body) > 0 ->
      %%MessageBody =  get_message_body(Packet),
      MessageBody = xml:get_tag_cdata(Tag),  
	  Type = get_message_type(Packet),
	  gen_server:cast(?PROCNAME, {message, Direction, From, To, Type, MessageBody});
      _ ->
...
-spec write_log(io:device(), direction(), jid(), jid(), binary(), binary()) -> ok.

write_log(IoDevice, Direction, From, To, Type, Body) ->
    Date = format_date(calendar:local_time()),
    Record = io_lib:format("~s [~s, ~s] ~s -> ~s ~s~n",
			   [Date, Direction, Type,
			    jlib:jid_to_string(From),
			    jlib:jid_to_string(To), Body]),
